...
def test_resume_agent_returns_string():
    ...
    with patch("agents.resume_agent._client.chat.completions.create",
               side_effect=_deepseek_seq("初稿bullets", "critique结果", "最终bullets")):
...
def test_resume_agent_calls_claude_three_times():
    ...
    with patch("agents.resume_agent._client.chat.completions.create",
               side_effect=_deepseek_seq("draft", "critique", "final")) as mock_create:
        run(JD_INFO, kb)
